...
class ChannelSurfManager {
    ...
    // v6: lower genre channel thresholds (≥15→2ch, ≥30→3ch, ≥50→4ch)
    private static readonly SESSION_KEY = 'channelSurf.queue.v6';
    ...
    private async _ensureQueue(config: SurfConfig): Promise<SurfQueue | null> {
        ...
        for (const theme of GENRE_THEMES) {
            ...
            for (let j = 0; j < numForGenre; j++) {
                ...
                genreChannels.push({
                    items: seededShuffle([ ...themeItems ], seed),
                    name,
                    tint: theme.tint,
                    accent: theme.accent
                });
        ...
        for (let i = 0; i < genreChannels.length; i++) {
            ...
            randomChannels.push({
                items: seededShuffle([ ...sortedItems ], seed),
                name: 'MIX',
                tint: pal.tint,
                accent: pal.accent
            });
